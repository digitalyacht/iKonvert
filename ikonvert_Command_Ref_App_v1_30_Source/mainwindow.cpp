...
bool wi_fi_udp = false;
bool wi_fi_tcp = false;
bool record = false;
...
void MainWindow::openSerialPort()
{
    ...
    if(p.name == "NavLink2(UDP)"){

        wi_fi_udp = true;
        wi_fi_tcp = false;

        showStatusMessage(tr("NavLink2(UDP) selected"));

        QString temp_put = QString("Connected to NavLink2(UDP)\r\n");
        m_console->putData(temp_put.toUtf8());
    ...
    }else if(p.name == "NavLink2(TCP)"){

        wi_fi_udp = false;
        wi_fi_tcp = true;

        showStatusMessage(tr("NavLink2(TCP) selected"));
        ...
        TcpSocket->connectToHost("192.168.1.1", 2000);

        if(!TcpSocket->waitForConnected(1000))
        {

            QString temp_put = QString("Error: not connected to NavLink2(TCP) !!!\r\n");
            m_console->putData(temp_put.toUtf8());
...
void MainWindow::connected(){
    ...
    m_console->putData("Connected to Navlink(TCP)\r\n");
...
void MainWindow::disconnected(){
    ...
    m_console->putData("TCP disconnected !!!\r\n");
    showStatusMessage(tr("TCP disconnected"));
...
void MainWindow::readyRead(){
    ...
    QByteArray tcp_data = TcpSocket->readLine();

    //m_console->putData(tcp_data);

    MainWindow::process_line(tcp_data);
...
void MainWindow::processPendingDatagrams()
{
    ...
    //m_status->setText(datagram);
    //m_console->putData(datagram);

    MainWindow::process_line(datagram);
...
void MainWindow::process_line(QByteArray buf)
{
      ...
      if(buf[0]=='$'){
          ...
          if((buf.mid(0, 18) == "$PDGY,NAK,4,SERIAL") || (buf.mid(0, 40) == "$PDGY,TEXT,iKonvert_serial_number_is_not")){

                program_serial();

          }else if(buf.mid(0, 13) == "$PDGY,000000,"){ // Network Status Sentence received
...
void MainWindow::send_init_command()
{
    ...
    writeData("$PDGY,N2NET_INIT,NORMAL\r\n");
...
void MainWindow::send_126992() // sending system time PGN command to the gateway
{
    if(initialised){
        ...
        writeData(system_time_PGN.toUtf8());
...
void MainWindow::program_serial()
{

    QByteArray serial_number = "$PDGY,Pr0d_C0nf!g,AB-";
    QDate dt = QDate::currentDate();
    QString dte = dt.toString("ddMMyy");
    serial_number.append(dte);
    serial_number.append("-0");
    quint32 rnd = QRandomGenerator::global()->bounded(10000, 99999);
    serial_number.append(QString::number(rnd));

    QMessageBox msgBox;
    msgBox.setWindowTitle("iKonvert");
    msgBox.setText("Programming iKonvert Serial Number\n\nChoose Device Type:");
    QAbstractButton *myYesButton = msgBox.addButton(trUtf8("ISO"), QMessageBox::YesRole);
    QAbstractButton *myNoButton = msgBox.addButton(trUtf8("USB"), QMessageBox::NoRole);
    msgBox.exec();

    if(msgBox.clickedButton() == myNoButton){

        serial_number.append(",USB\r\n");

    }else{

        serial_number.append(",ISO\r\n");

    }

    m_serial->write(serial_number);
    m_serial->waitForBytesWritten(1500);
    m_serial->write(serial_number);
    m_serial->waitForBytesWritten(1500);


    serial_number.remove(0, 18);
    QByteArray showserial = "Serial Number Programmed\n\n" + serial_number;

    QMessageBox msgBox2;
    msgBox2.setWindowTitle("iKonvert");
    msgBox2.setText(showserial);
    msgBox2.exec();
...
{
    QMessageBox::about(this, tr("About iKonvert Command Reference App v1.30 by Digital Yacht"),
                       tr("<b>iKonvert Command Reference App by Akos Kelemen<b>"));
}
...
void MainWindow::writeData(const QByteArray &data)
{
    if(wi_fi_udp){
        ...
        //socket->writeDatagram(data, QHostAddress::LocalHost, 2000); //write to udp
        ...
        socket.writeDatagram(datagram, QHostAddress::LocalHost, 2000);
